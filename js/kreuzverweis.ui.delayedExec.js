 ...
 */
function delayedExec(delay, theFunc, theQueue, destroyFunc) {
	if(delay == 0) {
		...
		var timer = setTimeout(function() {
			runIfLatest(theQueue, theFunc);
		}, delay);		
		while (window[theQueue].length > 0) {
			var outdatedTimer = window[theQueue].shift();
			console.debug("removing timer " + timer + " as more calls have been queued ...");
			clearTimeout(timer);			
		}
...
function runIfLatest(theQueue, theFunc) {
	...
	console.debug("running timer " + timer);
	theFunc.call();
}
